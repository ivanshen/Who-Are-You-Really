...
public class HighLowRenderer extends AbstractXYItemRenderer implements XYItemRenderer, Cloneable, PublicCloneable, Serializable {
    ...
    public HighLowRenderer() {
        ...
        this.tickLength = DateAxis.DEFAULT_AUTO_RANGE_MINIMUM_SIZE_IN_MILLISECONDS;
    }
            ...
            if (dataset instanceof OHLCDataset) {
                ...
                if (!(Double.isNaN(yHigh) || Double.isNaN(yLow))) {
                    ...
                    if (orientation == PlotOrientation.HORIZONTAL) {
                        g2.draw(new Double(yyLow, xx, yyHigh, xx));
                        entityArea = new Rectangle2D.Double(Math.min(yyLow, yyHigh), xx - NormalizedMatrixSeries.DEFAULT_SCALE_FACTOR, Math.abs(yyHigh - yyLow), DateAxis.DEFAULT_AUTO_RANGE_MINIMUM_SIZE_IN_MILLISECONDS);
                    } else if (orientation == PlotOrientation.VERTICAL) {
                        g2.draw(new Double(xx, yyLow, xx, yyHigh));
                        entityArea = new Rectangle2D.Double(xx - NormalizedMatrixSeries.DEFAULT_SCALE_FACTOR, Math.min(yyLow, yyHigh), DateAxis.DEFAULT_AUTO_RANGE_MINIMUM_SIZE_IN_MILLISECONDS, Math.abs(yyHigh - yyLow));
                    }
                }
                ...
                if (getDrawOpenTicks()) {
                    ...
                    if (!Double.isNaN(yOpen)) {
                        ...
                        if (orientation == PlotOrientation.HORIZONTAL) {
                            g2.draw(new Double(yyOpen, xx + delta, yyOpen, xx));
                        } else if (orientation == PlotOrientation.VERTICAL) {
                            g2.draw(new Double(xx - delta, yyOpen, xx, yyOpen));
                        }
                    }
                }
                if (getDrawCloseTicks()) {
                    ...
                    if (!Double.isNaN(yClose)) {
                        ...
                        if (orientation == PlotOrientation.HORIZONTAL) {
                            g2.draw(new Double(yyClose, xx, yyClose, xx - delta));
                        } else if (orientation == PlotOrientation.VERTICAL) {
                            g2.draw(new Double(xx, yyClose, xx + delta, yyClose));
                        }
                    }
                }
            } else if (item > 0) {
                ...
                if (!Double.isNaN(x0) && !Double.isNaN(y0) && !Double.isNaN(y)) {
                    ...
                    if (orientation == PlotOrientation.HORIZONTAL) {
                        g2.draw(new Double(yy0, xx0, yy, xx));
                    } else if (orientation == PlotOrientation.VERTICAL) {
                        g2.draw(new Double(xx0, yy0, xx, yy));
                    }
                } else {
                    return;
                }
