...
public class LongNeedle extends MeterNeedle implements Cloneable, Serializable {
    ...
    protected void drawNeedle(Graphics2D g2, Rectangle2D plotArea, Point2D rotate, double angle) {
        ...
        float midX = (float) (((double) minX) + (plotArea.getWidth() * 0.5d));
        float midY = (float) (((double) minY) + (plotArea.getHeight() * 0.8d));
        float y = maxY - (Axis.DEFAULT_TICK_MARK_OUTSIDE_LENGTH * (maxY - midY));
        if (y < minY) {
            y = minY;
        }
