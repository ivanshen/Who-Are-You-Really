...
public class CSV {
    ...
    private List extractColumnKeys(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (line.charAt(i) == this.fieldDelimiter) {
                if (fieldIndex > 0) {
                    keys.add(removeStringDelimiters(line.substring(start, i)));
                }
                start = i + 1;
                fieldIndex++;
            }
        }
        keys.add(removeStringDelimiters(line.substring(start, line.length())));
        return keys;
    }
    ...
    private void extractRowKeyAndData(String line, DefaultCategoryDataset dataset, List columnKeys) {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (line.charAt(i) == this.fieldDelimiter) {
                if (fieldIndex == 0) {
                    rowKey = removeStringDelimiters(line.substring(start, i));
                } else {
                    dataset.addValue(Double.valueOf(removeStringDelimiters(line.substring(start, i))), rowKey, (Comparable) columnKeys.get(fieldIndex - 1));
                }
                start = i + 1;
                fieldIndex++;
            }
        }
        dataset.addValue(Double.valueOf(removeStringDelimiters(line.substring(start, line.length()))), rowKey, (Comparable) columnKeys.get(fieldIndex - 1));
    }
